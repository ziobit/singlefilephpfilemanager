...
function to_rel(string $abs): string {
  $rootReal = realpath(BASE_ROOT);
  $root = str_replace('\\','/', (string)($rootReal ?: ''));
...
// --- PHP 7.2 helpers (mbstring optional + SameSite cookie) ---
function u_strtolower(string $s): string {
  return function_exists('mb_strtolower') ? (string)mb_strtolower($s, 'UTF-8') : strtolower($s);
}
function u_strpos(string $haystack, string $needle, int $offset = 0) {
...
function setcookie_samesite(string $name, string $value, int $expires, string $path, bool $secure, bool $httponly, string $samesite = 'Lax'): bool {
...
if (isset($_GET['search']) && is_authenticated()) {
  ...
  try {
    ...
    foreach ($it as $path => $info) {
      ...
      $needle = $caseSensitive ? $q : u_strtolower($q);
      if ($needle === '') { fclose($fh); continue; }
      ...
      while (!feof($fh)) {
        ...
        $hay = $caseSensitive ? $line : u_strtolower($line);
        if (u_strpos($hay, $needle) === false) continue;
...
if (isset($_GET['download_zip_folder']) && isset($_GET['f']) && is_authenticated()) {
  ...
  if ($token !== '') {
    ...
    setcookie_samesite('fileDownloadToken', $token, time() + 60, '/', $secure, false, 'Lax');
  }
  if ($abs && is_dir($abs)) {
    $trash = trash_abs();
